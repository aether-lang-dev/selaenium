...
# Third-party consumer example. Requires the INSTALLED `selenium_core` gem (from
...
def check_installed
  path = $LOADED_FEATURES.find { |f| f.end_with?('selenium_core.rb') }
  ...
  if path.nil? || path.include?("#{File::SEPARATOR}ruby#{File::SEPARATOR}lib#{File::SEPARATOR}")
    abort "FAIL: loaded selenium_core from #{path.inspect}, not the installed gem"
  end
end

def mode_ffi
  check_installed
  raise 'route mismatch' unless SeleniumCore.route('get') == 'POST /session/:sessionId/url'
  raise 'error_code mismatch' unless SeleniumCore.error_code('no such element') == 17
  loc = SeleniumCore.decode_by(SeleniumCore::By::ID, 'main')
  ...
  rescue SeleniumCore::WebDriverError => e
...
def mode_discovery
  ...
  check_installed
  gem_root = File.dirname($LOADED_FEATURES.find { |f| f.end_with?('selenium_core.rb') })
  native = File.join(gem_root, 'selenium_core', 'native')
  ...
  raise 'route mismatch' unless SeleniumCore.route('newSession') == 'POST /session'
...
def mode_live
  ...
  begin
    ...
    begin
      ...
      abort 'FAIL: text' unless d.find_element(SeleniumCore::By::ID, 'h').text == 'Hi'
