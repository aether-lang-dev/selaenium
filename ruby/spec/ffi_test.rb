...
class FFITest < Minitest::Test
  def test_route
    assert_equal 'POST /session/:sessionId/url', Selenium::WebDriver.route('get')
    assert_equal '', Selenium::WebDriver.route('nope')
  end

  def test_error_code
    assert_equal 17, Selenium::WebDriver.error_code('no such element')
    assert_equal 0, Selenium::WebDriver.error_code('')
  end

  def test_locator_css
    assert_equal({ 'using' => 'css selector', 'value' => 'div.foo' },
                 Selenium::WebDriver.decode_by(Selenium::WebDriver::By::CSS_SELECTOR, 'div.foo'))
  end

  def test_locator_id_rewrite
    assert_equal({ 'using' => 'css selector', 'value' => '*[id="main"]' },
                 Selenium::WebDriver.decode_by(Selenium::WebDriver::By::ID, 'main'))
  end

  def test_transport_failure
    err = assert_raises(Selenium::WebDriver::WebDriverError) do
