 ...
 *
 * Vendored from mainstream selenium-webdriver (npm) lib/input.js for perfect ABI
 * parity: Key (singular, with Key.chord), Button, Origin, FileDetector, and the
 * full Actions builder (Keyboard/Pointer/Wheel devices). Only the executor seam
 * differs — upstream drives a command.Executor; here Actions.perform()/clear()
 * call this binding's WebDriver.performActions()/clearActions() (blocking FFI
 * under the hood, awaitable ABI on top). WebElement pointer/scroll origins are
 * serialized to their W3C element reference before dispatch.
 */
...
class Actions {
  ...
  clear() {
    ...
    return this.executor_.clearActions()
  }
  ...
  async perform() {
    ...
    // Serialize any WebElement origin to its W3C element reference before
    // dispatch (upstream's toWireValue does this via Symbols.serialize; this
    // binding sends the actions JSON straight to the engine).
    await this.executor_.performActions(serializeOrigins(_actions))
  }
...
// The W3C element-reference key. A pointer/scroll action's `origin` may be a
// WebElement; the engine expects it as { element-key: id } on the wire.
const W3C_ELEMENT_KEY = 'element-6066-11e4-a52e-4f735466cecf'

// Replace any WebElement `origin` inside a device's action list with its W3C
// element reference. Detected structurally (an object exposing an `id`) to avoid
// a require cycle with ./webdriver.
function serializeOrigins(deviceSequences) {
  return deviceSequences.map((seq) => {
    if (!Array.isArray(seq.actions)) return seq
    const actions = seq.actions.map((a) => {
      const origin = a.origin
      if (origin && typeof origin === 'object' && typeof origin.id === 'string') {
        return { ...a, origin: { [W3C_ELEMENT_KEY]: origin.id } }
      }
      return a
    })
    return { ...seq, actions }
